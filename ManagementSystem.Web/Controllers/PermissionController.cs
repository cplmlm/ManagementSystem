...
using ManagementSystem.Extensions.Authorizations.Helpers;
...
namespace ManagementSystem.Controllers
{
   ...
   [Authorize(Permissions.Name)]
    public class PermissionController : BaseApiController
    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<MessageModel<List<Permission>>> GetTreeTable(long pid = 0,int number=0, string key = "")
        {
            ...
            {
                permissions = permissionsList.Where(a => a.Pid ==pid).OrderBy(a => a.OrderSort).ToList();
            }
        ...
        public async Task<MessageModel<string>> Put([FromBody] Permission permission)
        {
            ...
            {
                bool success = await _permissionServices.Update(permission);
        ...
        public async Task<MessageModel<string>> Delete(long id)
        {
            ...
            if (id > 0)
            {
               ...
               bool success = await _permissionServices.Update(userDetail);
                if (success)
                {
                    data.Message = "删除成功";
        ...
        public async Task<MessageModel<string>> BatchPost([FromBody] List<Permission> permissions)
        {
          ...
          bool  success = ids.IsNotEmptyOrNull();
            if (success)
            {
                data.Data= ids;
        ...
        public async Task<MessageModel<List<Permission>>> MigratePermission(string action = "", string token = "", string gatewayPrefix = "", string swaggerDomain = "", string controllerName = "", long pid = 0, bool isAction = false)
        {
            ...
            data.Status = ResultStatus.Success;
            return data;
        }
    }
