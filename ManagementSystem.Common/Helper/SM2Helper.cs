...
namespace ManagementSystem.Common.Helper
{
    public  class SM2Helper
    {
        ...
        private static ICipherParameters _privateKeyParameters;
        private static string _pubkey;
        private static ICipherParameters _publicKeyParameters;

        static SM2Helper()
        {
            _privkey=AppSettings.app(new string[] { "SM2Config", "PrivateKey" });
            _pubkey = AppSettings.app(new string[] { "SM2Config", "PublicKey" });
            _mode =(Mode) AppSettings.app(new string[] { "SM2Config", "Mode" }).ObjToInt();
        }
        private static ICipherParameters PrivateKeyParameters
        {
            get
            {
                try
        ...
        private static ICipherParameters PublicKeyParameters
        {
            get
            {
                try
        ...
        public static string Decrypt(string cipherText)
        {
            try
            {
                byte[] data = HexStrToBytes(cipherText);
                if (_mode == Mode.C1C3C2)
                    data = C132ToC123(data);
                var sm2 = new SM2Engine(new SM3Digest());
                sm2.Init(false, PrivateKeyParameters);
                var a = sm2.ProcessBlock(data, 0, data.Length);
                string output = Encoding.UTF8.GetString(a);
                return output;
            }
            catch (Exception ex)
            {
