...
namespace ManagementSystem.Common.Helper
{
    ...
    public static class RecursionHelper
    {
        ...
        /// 行政区划递归子级数据
        /// </summary>
        /// <param name="areaTrees"></param>
        /// <param name="curItem"></param>
        public static void LoopAreaChildren(List<AreaTree> areaTrees, AreaTree curItem)
        {
            var subItems = areaTrees.Where(x => x.ParentId == curItem?.Value.ObjToLong()).ToList();
            if (subItems.Count > 0)
            {
                curItem.Children = new List<AreaTree>();
                curItem.Children.AddRange(subItems);
            }
            else
            {
            ...
            foreach (var subItem in subItems)
            {
                LoopAreaChildren(areaTrees, subItem);
            }
        }
