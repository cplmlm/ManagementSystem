...
namespace ManagementSystem.Services.Base
{
    public class BaseServices<TEntity> : ITransientInterface, IBaseServices<TEntity> where TEntity : class, new()
    {
        ...
        {
            return await BaseDal.Add(entity, isPrimaryKey);
        }
        ...
        /// <returns></returns>
        public async Task<List<TResult>> Query<TResult>(Expression<Func<TEntity, TResult>> expression, Expression<Func<TEntity, bool>> whereExpression, Expression<Func<TEntity, object>> groupExpression, string orderByFileds)
        {
            return await BaseDal.Query(expression, whereExpression,groupExpression, orderByFileds);
        }
