...
using ManagementSystem.Common.Swagger;
using ManagementSystem.Model.Models;

namespace ManagementSystem.Extensions.Authorizations.Policys
{
    /// <summary>
    /// 权限授权处理器
    /// </summary>
            ...
            if (httpContext != null)
            {
                ...
                if (defaultAuthenticate != null)
                {
                    ...
                    if (result?.Principal != null || isTestCurrent || httpContext.IsSuccessSwagger())
                    {
                        ...
                        if (user == null)
                        {
                            _user.MessageModel = new ApiResponse(ResultStatus.CODE401, "用户不存在或已被删除").MessageModel;
                            context.Fail(new AuthorizationFailureReason(this, _user.MessageModel.Message));
                            return;
                        }
                        if (user.IsDeleted)
                        {
                            _user.MessageModel = new ApiResponse(ResultStatus.CODE401, "用户已被删除,禁止登陆!").MessageModel;
                            context.Fail(new AuthorizationFailureReason(this, _user.MessageModel.Message));
                            return;
                        }
                        if (!user.Enable)
                        {
                            _user.MessageModel = new ApiResponse(ResultStatus.CODE401, "用户已被禁用!禁止登陆!").MessageModel;
                            context.Fail(new AuthorizationFailureReason(this, _user.MessageModel.Message));
                            return;
                        }
                        // 判断token是否过期，过期则重新登录
                        ...
                        if (value != null)
                        {
                            ...
                            {
                                _user.MessageModel = new ApiResponse(ResultStatus.CODE401, "很抱歉,授权已失效,请重新授权")
                                    .MessageModel;
                                context.Fail(new AuthorizationFailureReason(this, _user.MessageModel.Message));
                                return;
                            }
