...
namespace ManagementSystem.Repository.Base
{
    public class BaseRepository<TEntity> : ITransientInterface ,IBaseRepository<TEntity> where TEntity: class, new()
    {
        ...
        /// <param name="isPrimaryKey">是否自定义是主键</param>
        /// <returns></returns>
        public async Task<long> Add(TEntity entity, bool isPrimaryKey)
        {
        ...
        public async Task<bool> Update(string sql, SugarParameter[] parameters)
        {
        ...
        /// <param name="groupExpression">分组字段</param>
        /// <param name="orderByFields">排序条件</param>
        /// <returns></returns>
        public async Task<List<TResult>> Query<TResult>(Expression<Func<TEntity, TResult>> expression, Expression<Func<TEntity, bool>> whereExpression, Expression<Func<TEntity, object>> groupExpression, string orderByFields)
        {
            return await _db.Queryable<TEntity>().GroupByIF(groupExpression != null, groupExpression).OrderByIF(!string.IsNullOrEmpty(orderByFields), orderByFields).WhereIF(whereExpression != null, whereExpression).Select(expression).ToListAsync();
        }
        ...
        public async Task<PageModel<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression, int pageIndex = 1, int pageSize = 20, string orderByFields = null)
        {
            ...
            string a = _db.Queryable<TEntity>()
                .OrderByIF(!string.IsNullOrEmpty(orderByFields), orderByFields)
                .WhereIF(whereExpression != null, whereExpression).ToSqlString();
        ...
        /// <param name="whereLambda">查询表达式 (w1, w2) =>w1.UserNo == "")</param> 
        /// <returns>值</returns>
        public async Task<TResult> QueryMuchSingle<T, T2, TResult>(
            Expression<Func<T, T2, object[]>> joinExpression,
            Expression<Func<T, T2, TResult>> selectExpression,
            Expression<Func<T, T2, bool>> whereLambda = null) where T : class, new()
        {
            if (whereLambda == null)
            {
                return await _db.Queryable(joinExpression).Select(selectExpression).MergeTable().SingleAsync();
            }

            return await _db.Queryable(joinExpression).Where(whereLambda).Select(selectExpression).MergeTable().SingleAsync();
        }
        /// <summary> 
        ///查询-两表分组查询
        /// </summary> 
        ...
        /// <param name="joinExpression">关联表达式 (join1,join2) => new object[] {JoinType.Left,join1.UserNo==join2.UserNo}</param> 
        /// <param name="selectExpression">返回表达式 (s1, s2) => new { Id =s1.UserNo, Id1 = s2.UserNo}</param>
        /// <param name="groupExpression">group表达式(g1) => g1.UserNo)</param>
        /// <param name="whereLambda">查询表达式 (w1, w2) =>w1.UserNo == "")</param> 
        /// <returns>值</returns>
        public async Task<List<TResult>> QueryMuchGroup<T, T2, TResult>(
            Expression<Func<T, T2, object[]>> joinExpression,
            Expression<Func<T, T2, TResult>> selectExpression,
...

        {
            ...
            return await _db.Queryable(joinExpression).GroupBy(groupExpression).Where(whereLambda).Select(selectExpression).ToListAsync();
        }
        /// <summary> 
        ///查询-多表查询
        /// </summary> 
        ...
        /// <param name="whereLambda">查询表达式 (w1, w2) =>w1.UserNo == "")</param> 
        /// <returns>值</returns>
        public async Task<List<TResult>> QueryMuchGroup<T, T2, T3, TResult>(
            Expression<Func<T, T2, T3, object[]>> joinExpression,
            Expression<Func<T, T2, T3, TResult>> selectExpression,
            Expression<Func<T, object>> groupExpression,
            Expression<Func<T, T2, T3, bool>> whereLambda = null) where T : class, new()
        ...
        /// <summary>
        /// 多表联合查询-分页
        ///增加MergeTable解决中文提示 : 多表查询存在别名不一致,请把Where中的wlzxtt改成wlzx就可以了
        /// </summary>
        /// <typeparam name="T">实体1</typeparam>
        /// <typeparam name="T2">实体1</typeparam>
        /// <typeparam name="TResult">返回对象</typeparam>
